...
class CameraPoseDataset(Dataset):
    ...
    def __init__(self, dataset_path, rssi_file, labels_file, baseline_transform=None, wifi_transform=None, equalize_scenes=False, experiment='', mode=''):
        """
        ...
        if experiment == 'bicubic':
            self.rssi_values_lr = read_lr_rssi_file(rssi_file)
            self.rssi_values = np.zeros((len(self.rssi_values_lr), 56, 56, 1), dtype=np.uint8)
            for idx in range(len(self.rssi_values_lr)):
                rssi_value = hr_restore_transform()(self.rssi_values_lr[idx])
                rssi_value = rssi_value.cpu().numpy() * 255
                rssi_value = rssi_value.transpose((1,2,0))
                self.rssi_values[idx] = rssi_value
            self.rssi_values = np.repeat(np.repeat(self.rssi_values,4,axis=1),4,axis=2)
        else:
